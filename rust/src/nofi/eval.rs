...
impl RustApplication {
    ...
    pub fn new() -> Self {
        ...
        let gun_actions = String::from_utf8(gun_actions_output.stdout).expect("Failed to convert output to string");
        ...
        for line in gun_actions.split("\n") {
            ...
            graphics_index.insert(spell_id.to_string(), (SpellType::from_int(spell_type.parse().unwrap()), spell_path.to_string()));
        }
        ...
        let mut autocomplete = SearchIndexBuilder::default()
            ...
            .dump_keyword(None)
            .build();
        let index_keys = spell_dictionary.keys().map(|x| x.clone()).collect::<Vec<String>>();
    ...
    fn get_default(&self) -> DynamicImage {
        match image::ImageReader::open(&format!("{}/data/ui_gfx/inventory/full_inventory_box.png", &self.args.data_path)) {
    ...
    for token in tokens {
        ...
        let (spell_bg, spell_sprite) = match self.graphics_index.get(token) {
            Some((spell_type, sprite_path)) => (
                image::ImageReader::open(&format!("{}/{}", &self.args.data_path, spell_type.bg_path())).unwrap().decode().unwrap(),
                image::ImageReader::open(&format!("{}/{}", &self.args.data_path, sprite_path)).unwrap().decode().unwrap(),
            ),
            None => (
